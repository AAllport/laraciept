...
class LinePrintTest extends TestCase
{
    public function testBasicBuilder()
    {
        ...
        LinePrint::make()
            ->line("Left")
            ->line("Center", fn($s) => $s->align(LineSectionSettings::ALIGN_CENTER))
            ->line("Right", fn($s) => $s->align(LineSectionSettings::ALIGN_RIGHT))
...
$printer->close();
    ...
    public function testAlignRight()
    {
        ...
        LinePrint::make()
            ->line("Right Aligned",fn(LineSectionSettings $s)=>$s->align(LineSectionSettings::ALIGN_RIGHT))
            ->render($printer);
